...
extern const char *const input_tok_type_str[__TOK_COUNT];
...
int input_tok_deb_dprint(int fd, InputTok input_tok) {
    ...
    if (input_tok->type == TOK_NONE || input_tok->type == TOK_EOF) {
    ...
    } else if (input_tok->type == TOK_EQ_SPLIT) {
        String Cleanup(string_drop) string = string_new();
        ...
        return string_dprint(fd, string);
    } else if (input_tok->type == TOK_MINUS_SPLIT) {
        String Cleanup(string_drop) string = string_new();
        string_fmt(string, "{type: '%s', ln: %zu, col: %zu, slice: '%.*s'}",
