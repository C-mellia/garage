...
static inline __attribute__((unused))
InputTok input_tok_stream_next(struct stream *ch_stream, Coord coord);
static inline __attribute__((unused))
int input_tok_should_end(InputTok input_tok);
static inline __attribute__((unused))
void input_tok_stream_add_ch(InputTok input_tok, Stream ch_stream, StreamStatus3 status, Array errs, char *peek);
static inline __attribute__((unused))
...
static InputTok input_tok_stream_next(Stream ch_stream, Coord coord) {
    ...
    for (; !status->should_stop;) {
        ...
        input_tok_stream_add_ch(input_tok, ch_stream, status, errs, peek);
        ...
        for (int i = 0; i < status->should_consume; ++i) {
            ...
            arr_push_back((void *)input_tok->arr, peek);
        }
...
static void input_tok_stream_add_ch(InputTok input_tok, Stream ch_stream, StreamStatus3 status, Array errs, char *peek) {
...
#define get_type_str (input_tok->type >= 0 && input_tok->type < __INPUT_TOK_COUNT? input_tok_type_str[input_tok->type]: "Invalid InputInputToken Type")
    ...
    switch(input_tok->type) {
        ...
        case INPUT_TOK_EQ_SPLIT: {
            if (!peek) {
                ...
                status->should_stop = 1, status->should_consume = 1;
            } else if (ch_is_text(peek)) {
